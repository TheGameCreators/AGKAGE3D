...
Function LAG_AddGadgetItem(GadgetId, Position, txt$, imgId)
    ...
    */
    ...
    if LAG_D_Gadget[GadgetId].Typ =  LAG_C_TYPPANEL or LAG_D_Gadget[GadgetId].Typ =  LAG_C_TYPLISTICON
		...
		// si position =-1, on crée le suivant en dynamique
		inc LAG_D_Gadget[GadgetId].NbGadgetItem // increment the number of item, for the panel
		if position = -1
		...
		select LAG_D_Gadget[GadgetId].Typ 
			...
			case LAG_C_TYPLISTICON, LAG_C_TYPCOMBOBOX
				...
				h2 = 32 // the size of gadget item by default
				
				// to know the size of the listicon
				...
				w9 = 32 // for slider // de la place à droite pour gadget, c'est pour l'ascenceur !
				
				// position gadget parent 
				...
				FoldStart // create icone with image
				if imgID > 0				
        ...
        if LAG_D_Gadget[GadgetId].Typ =  LAG_C_TYPPANEL
			...
			xa = x + w/2
			ya = 1 + y +h/2-8
			LAG_D_GadgetItem[i].TextId =  LAG_CreateText(txt$, xa, ya, depth, 1, 18)
			LAG_D_GadgetItem[i].TextX = xa
			LAG_D_GadgetItem[i].Texty = ya
		...
		elseif LAG_D_Gadget[GadgetId].Typ =  LAG_C_TYPLISTICON
			...
			n =  LAG_CreateText(txt$, xa, ya, depth, 1, 16)
			// SetTextAlignment(LAG_D_GadgetItem[i].TextId,1)
			LAG_D_GadgetItem[i].TextX = xa //LAG_D_GadgetItem[i].spr1x + 10 + h2 + LAG_D_GadgetItem[i].Parentx  // x+10+h2
			LAG_D_GadgetItem[i].TextY = ya
			LAG_D_GadgetItem[i].TextId = n
			SetTextColor(n,LAG_TextColorR,LAG_TextColorG,LAG_TextColorB,255)			
			SetTextScissor(n, xa, y1+5, xa+w1-w9, y1+h1-5)
			...
			select LAG_D_Gadget[i].typ 
				...
				case LAG_C_TYPLISTICON
					...
					*/
					...
					// textid 
					n = LAG_D_GadgetItem[g].Textid
					x2 = LAG_D_GadgetItem[g].textX + LAG_D_GadgetItem[g].parentX // + 10 + w2
					//y2 = LAG_D_GadgetItem[g].TextY + LAG_D_GadgetItem[g].parentY - LAG_d_Gadget[i].CurY + oldy
					y2 = LAG_D_GadgetItem[g].TextY - LAG_d_Gadget[i].CurY // + oldy
					SetTextDepth(n, lag__d)			
...
Function LAG_SetGadgetItemReSizeAll(gadgetId,x,y,w,h)
	...
	for i= 0 to LAG_D_GadgetItem.length
		...
		if LAG_D_GadgetItem[i].ParentID  = gadgetId
			...
			// if LAG_D_Gadget[k].typ = LAG_C_TYPLISTICON
					
				// LAg_SetGadgetItemPosition(i, x1, y1, w, h)
...
Function LAG_SetGadgetItemSize_(gadgetId,pos,x,y,w,h)
		...
		if  LAG_D_GadgetItem[j].ParentID = i
			...
			Select LAG_D_Gadget[i].typ
				...
				case LAG_C_TYPPANEL
					...
					LAg_SetGadgetItemPosition(j, x1, y1, 0, 0)
				...
				case LAG_C_TYPLISTICON
					...
					LAg_SetGadgetItemPosition(j, x1, y1, w, h)
...
Function LAg_SetGadgetItemPosition(j, x1, y1, w, h)
	
	//	to set the new position of each gadget item
	...
	if LAG_D_GadgetItem[j].attribute = LAg_ListIcon_DisplayMode
		if LAG_D_GadgetItem[j].attributeValue >= LAg_ListIcon_SmallIcon	
			oldY = 0							
		endif
	endif	
...
Function LAG_SetGadgetAttribute(GadgetID, Attribute, value)
	...
	// For attribute and value, see LAg_constants.agc //***** Attributes
	...
	for i= 0 to LAG_D_GadgetItem.length
		...
		if LAG_D_GadgetItem[i].ParentID  = gadgetId
			...
			w1 = LAG_D_GadgetItem[i].w // GetSpriteWidth(n)
			...
			if attribute = LAg_ListIcon_DisplayMode
				...
				// LAG_D_GadgetItem[i].w = w
				LAG_D_GadgetItem[i].h = h
				SetSpriteSize(n, w1, h) // to select				
				SetSpriteSize(n1, w, h) // the icon
				
				
				if square = 1
					
					a = 20
					nbx = LAG_D_GadgetItem[i].w / (w+a)
					
					x = mod(j, nbx) * (w+a)
					y = a + j/nbx * (w+a) 
					SetSpritePosition(n1, x, y)
					SetSpritePosition(n, x, y)
					SetTextPosition(n, x, y) 
					
					LAG_D_GadgetItem[i].originx = x
					LAG_D_GadgetItem[i].originy = y
					LAG_D_GadgetItem[i].spr1x = x
					LAG_D_GadgetItem[i].spr1y = y
					LAG_D_GadgetItem[i].TextX = x -(w + a)*0.5
					LAG_D_GadgetItem[i].Texty = y +(w + a)
