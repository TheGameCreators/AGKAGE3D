	...
	*/
	...
	For i=0 to Folder$.length
		...
		if Folder$[i] <> ""
			...
			SetSpritePosition(j,x,y+i*22) 		
			SetSpriteColor(j,60,60,60,255)
			SetSpriteDepth(j,1)
			SetSpriteScissor(j,x,y,x+w6-20,y+h6)
			FixSpriteToScreen(j,1)
			
			TextFolder[i] = CreateText(Folder$[i])
			j = TextFolder[i]
			SetTextPosition(j,x,y+2+i*22)
			SetTextSize(j,15)
			SetTExtDepth(j,1)
			SetTextScissor(j,x,y,x+w6-50,y+h6)
			FixTExtToScreen(j,1)
...
Function LAG_OpenFile(Title$, DefaultFiles$, Pattern$, Position,width,height,view)
	...
	*/
	...
	FreeToolTips()
		...
		y = GetSpriteY(Menu)+80
		a=6
		
		
		// the background for the files
		w6 = width-220 : h6= height-200
		...
		// Folder list	
		w = 150
		x1 = x-a-w-20 : y1 = y-a : w7 = w+a*2 
		i4 = LAG_CreateMenuBox(x1,y1, w7, h6+a*2, LAG_C_ICORNER2, 255) : inc nmb 
		FolderList = LAG_GetMenuboxSprite(i4) // to know the sprite of the menubox, to free the menubox at the end of the function
		
		
		
		// create editobox for filename
		w8 = 100
		w = w6 -w8-a*4
					...
					For i=0 to SpFolder.length
						if butonTest = SpFolder[i]
							...
							if OldFolderId <> IdFile
								...
								next
								// create the new file
								path$ = path1$+ GetTextString(TextFolder[i])+"/"
