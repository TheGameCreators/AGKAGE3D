...
uniform mediump vec2 Scale; // a good scale is 0.5 and 0.5
...
{ 
    ...
    posVarying = pos.xyz;
    normalVarying = norm;
	float int_ = 0.50 ; // Scale.x;
	float int2_ = 0.50 ; // Scale.y;
	uvNormal = - normalize((agk_WorldViewProj * vec4(normal, 0.0)).xyz).xy*int_+int2_; // normal
	//uvNormal = - normalize((agk_WorldViewProj * vec4(normal, 0.0)).xyz).xy * 0.45 + 0.50 ; // normal
